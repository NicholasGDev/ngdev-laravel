...
pub fn generate(project_root: &str) -> Result<()> {
    ...
    // ── 5. Controllers ────────────────────────────────────────────────────────
    println!("  {}", style("[ 4/4 ] Gerando Controllers (App\\Http\\Controllers\\Estoque)...").cyan().bold());
    ...
    write_file(&routes_dir.join("api_erp_estoque.php"), tmpl::ROUTES)?;

    // ── Resumo ────────────────────────────────────────────────────────────────
    println!("  {}", style("══════════════════════════════════════════════════════").dim());
    println!("  {}", style("  ERP Estoque MVCS gerado com sucesso!").green().bold());
    println!("  {}", style("══════════════════════════════════════════════════════").dim());
    println!();
    ...
    println!("    require __DIR__.'/api_erp_estoque.php';");
    println!();
    println!("  {} Módulos gerados:", style("MÓDULOS:").cyan().bold());
    println!("    Armazem · PosicaoEstoque · Fornecedor · Produto");
    println!("    Lote · PedidoCompra · ItemPedidoCompra");
    println!("    MovimentacaoEstoque (Kardex) · Inventario · ContagemInventario");
    println!();
